...
public class CannonCustomization : MonoBehaviour {
	...
	void Update ()
    {
        if (isInLobby) {    //If the player is in the Lobby Scene.
            LobbyInputs();
        }
    }
    ...
    void LobbyInputs()
    {
        ...
        if (rewiredPlayer.GetButtonDown("IncreaseRotationSpeed")) { //If the player presses 'Right' on the D Pad.
        ...
        if (rewiredPlayer.GetButtonDown("DecreaseRotationSpeed")) { //If the player presses 'Left' on the D Pad.
            cannon.ChangeRotationSpeed(-rotationSpeedIncrement);
            if (sensitivity > 1)
                sensitivity--;
